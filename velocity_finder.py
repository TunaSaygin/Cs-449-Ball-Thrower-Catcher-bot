...
## @brief Computes the initial point for the throwing motion based on the release point and velocity.
...
#  @param trajectory_array List to store the computed trajectory points (optional).
#  @return The computed initial position if found.
def find_initial_point_from_release(C: ry.Config, release_position, release_velocity, height_threshold=0.2, trajectory_array: list = None):
    ...
    while position[2] > height_threshold:
        ...
        if trajectory_array is not None:
            trajectory_array.append(position.copy())
...
## @brief Computes the optimal velocity for throwing the object to the bin.
#  @param C Robot configuration (KOMO simulation object).
#  @return A tuple containing the optimal velocity vector [vx, vy, vz] and a boolean indicating if the target is inverted.
def find_velocity(C: ry.Config):
    ...
    # Adjust bin position for inverted targets
